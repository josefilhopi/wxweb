...
int MD5( char *InputText, ULONG Size, char *OutPutText );
...
int MD5( char *InputText, ULONG Size, char *digest )
{
   ULONG i;
   BYTE dststr[16];
   ...
   if (InputText)
   {
      hbcc_md5(InputText,Size,dststr);
      for (i=0;i<16;i++)
      {
...
HB_FUNC(MD5)
{
   ...
   {
      char OutPutText[40];
      char * srcstr=(BYTE *) hb_itemGetCPtr(phbstr);
      ULONG srclen=hb_itemGetCLen(phbstr);
      
      if (MD5(srcstr,srclen,OutPutText))
         hb_retclen(OutPutText,32);
      else
